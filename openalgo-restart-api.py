...
.instance{background:#f8f9fa;padding:15px;margin:10px 0;border-radius:8px;display:flex;justify-content:space-between;align-items:center;border-left:4px solid #667eea}
.instance-name{font-weight:600;color:#333}
...
document.getElementById('summary').innerHTML=`<p>Total: ${instances.length} | Running: ${Object.values(status.instances||{}).filter(s=>s==='active').length}</p>`;
const html=instances.map(inst=>{
const s=status.instances?.[inst]||'unknown';
const active=s==='active';
return`<div class="instance"><div><div class="instance-name">${inst}<span class="badge ${active?'badge-active':'badge-inactive'}">${active?'● Active':'● Inactive'}</span></div><div class="status"><span class="${active?'active':'inactive'}">Status: ${s}</span></div></div><div><button class="btn btn-restart" onclick="restart('${inst}')">Restart</button> ${active?`<button class="btn btn-stop" onclick="stop('${inst}')">Stop</button>`:`<button class="btn btn-start" onclick="start('${inst}')">Start</button>`}</div></div>`;
}).join('');
